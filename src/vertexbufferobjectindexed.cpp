...
{
	glBufferData(GL_ARRAY_BUFFER, m_vertexData.size(), m_vertexData.data(), iUsageHint);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_indexData.size(), m_indexData.data(), iUsageHint);
	m_dataUploaded = true;
	m_vertexData.clear();
	m_indexData.clear();
}
